...
void LeptogenesisBE::Set_Temperatures(REAL Ti, REAL Tr, REAL Tf,
                                      REAL TInflation) {
...
VD LeptogenesisBE::dYdX0(REAL x, VD y) {
    ...
    res = z * z * R_Calculator.Ri(Temp, y) / Entropy_Density(Temp) /
          EMDEvo.Get_Hubble_at_T_Period(EMD::ERDE, Temp1);
    return res;
}
...
VD LeptogenesisBE::dYdX2(REAL x, VD y) {
    ...
    REAL entropy_term = 20.0 * EMDEvo.Get_Delta() / 3.0 *
                        pow(HEPatZ / HRDati, 2) * HRDatr / HEPat1 * pow(z, 4) /
                        zr * sati / satz * exp(-5.0 * HRDatr / HEPatZ / 3.0);
    res = 8.0 * z * z * z * z * R_Calculator.Ri(Temp, y) / satz /
              EMDEvo.Get_Hubble_at_T_Period(EMD::EPE, Temp1) -
          y * entropy_term;
    return res;
}
...
VD LeptogenesisBE::dYdX3(REAL x, VD y) {
    ...
    REAL Temp1 = R_Calculator.Get_Temperature_from_Z(1);
    res = z * z * R_Calculator.Ri(Temp, y) / Entropy_Density(Temp) /
          EMDEvo.Get_Hubble_at_T_Period(EMD::RDE, Temp1);
    return res;
}
