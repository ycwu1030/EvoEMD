...
LeptogenesisRate::LeptogenesisRate() : DOF(5), MNR1(3000), MNR2(3000), MNR3(1e10), MCHI(100), MS(300), LamX(1.0), Yeq(5)
{
...
void LeptogenesisRate::Set_LambdaX(REAL lamx)
{
    LamX = lamx;
}
...
void LeptogenesisRate::Calc_N_Width()
{
    ...
    if (MNR1 > MCHI + MS)
    {
        GammaN1 += MNR1 / 32.0 / M_PI * 2 * LamX * LamX * (1.0 + (MCHI * MCHI - MS * MS) / MNR1 / MNR1) * SQkaller(1.0, MCHI * MCHI / MNR1 / MNR1, MS * MS / MNR1 / MNR1);
    }
    if (MNR2 > MCHI + MS)
    {
        GammaN2 += MNR2 / 32.0 / M_PI * 2 * LamX * LamX * (1.0 + (MCHI * MCHI - MS * MS) / MNR2 / MNR2) * SQkaller(1.0, MCHI * MCHI / MNR2 / MNR2, MS * MS / MNR2 / MNR2);
    }
}
...
void LeptogenesisRate::Calc_Gammas(REAL Temp)
{
    ...
    gamma_Ni_Ni_S_S[0] = Calc_NNSS_Gamma(Temp, 0);
    gamma_Ni_Ni_S_S[1] = Calc_NNSS_Gamma(Temp, 1);
...
{
    REAL RN1 = eps1 * gamma_Ni_L_Phi[0] * (y[1] / Yeq[1] - 1.0) - gamma_Ni_L_Phi[0] * y[0] / 2.0 / Yeq[0];
    REAL RN2 = eps2 * gamma_Ni_L_Phi[1] * (y[2] / Yeq[2] - 1.0) - gamma_Ni_L_Phi[1] * y[0] / 2.0 / Yeq[0];
    return RN1 + RN2;
}
...
REAL LeptogenesisRate::R1(REAL Temp, VD y)
{
    ...
    RN1 += -gamma_Ni_Ni_Chi_Chi[0] * (y[1] * y[1] / Yeq[1] / Yeq[1] - y[3] * y[3] / Yeq[3] / Yeq[3]);
    RN1 += -gamma_Ni_Ni_S_S[0] * (y[1] * y[1] / Yeq[1] / Yeq[1] - y[4] * y[4] / Yeq[4] / Yeq[4]);
    // if (MNR1 > MCHI + MS)
...
REAL LeptogenesisRate::R2(REAL Temp, VD y)
{
    ...
    RN2 += -gamma_Ni_Ni_Chi_Chi[1] * (y[2] * y[2] / Yeq[2] / Yeq[2] - y[3] * y[3] / Yeq[3] / Yeq[3]);
    RN2 += -gamma_Ni_Ni_S_S[1] * (y[2] * y[2] / Yeq[2] / Yeq[2] - y[4] * y[4] / Yeq[4] / Yeq[4]);
    // if (MNR2 > MCHI + MS)
    // {
...
REAL LeptogenesisRate::R3(REAL Temp, VD y)
{
    ...
    RCHI += gamma_Ni_Ni_Chi_Chi[0] * (y[1] * y[1] / Yeq[1] / Yeq[1] - y[3] * y[3] / Yeq[3] / Yeq[3]);
    RCHI += gamma_Ni_Ni_Chi_Chi[1] * (y[2] * y[2] / Yeq[2] / Yeq[2] - y[3] * y[3] / Yeq[3] / Yeq[3]);
    // if (MNR1 > MCHI + MS)
    // {
...
REAL LeptogenesisRate::R4(REAL Temp, VD y)
{
    ...
    RS += gamma_Ni_Ni_S_S[0] * (y[1] * y[1] / Yeq[1] / Yeq[1] - y[4] * y[4] / Yeq[4] / Yeq[4]);
    RS += gamma_Ni_Ni_S_S[1] * (y[2] * y[2] / Yeq[2] / Yeq[2] - y[4] * y[4] / Yeq[4] / Yeq[4]);
...
{
    return sqrt((x - pow(sqrt(y) + sqrt(z), 2)) * (x - pow(sqrt(y) - sqrt(z), 2)));
}
